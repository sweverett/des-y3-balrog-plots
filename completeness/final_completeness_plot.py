...
if __name__ == '__main__':
    ...
    njack = 50
    # g:
    ax.errorbar(data['bin_centers'],data['band_detfracs'][0,:],data['detfrac_err']*np.sqrt(ntiles*1./njack),label='g',marker='o',color='blue',fmt='s')
    xdeep_g,detfrac_deep_g = np.loadtxt("deep_g.dat",unpack=True,skiprows=1)
    ax.plot(xdeep_g,detfrac_deep_g,color='blue')

    ax.errorbar(data['bin_centers'],data['band_detfracs'][1,:],data['detfrac_err']*np.sqrt(ntiles*1./njack),label='r',marker='o',color='green',fmt='s')
    xdeep_r,detfrac_deep_r = np.loadtxt("deep_r.dat",unpack=True,skiprows=1)
    ax.plot(xdeep_r,detfrac_deep_r,color='green')

    ax.errorbar(data['bin_centers'],data['band_detfracs'][2,:],data['detfrac_err']*np.sqrt(ntiles*1./njack),label='i',marker='o',color='orange',fmt='s')
    xdeep_i,detfrac_deep_i = np.loadtxt("deep_i.dat",unpack=True,skiprows=1)
    ax.plot(xdeep_i,detfrac_deep_i,color='orange')

    ax.errorbar(data['bin_centers'],data['band_detfracs'][3,:],data['detfrac_err']*np.sqrt(ntiles*1./njack),label='z',marker='o',color='red',fmt='s')
